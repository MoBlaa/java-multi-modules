...
public class UserRegisterInteractorTest extends TestBase {
  ...
  @Mock private UserDsGateway gateway;
  @Mock private UserPresenter presenter;
  ...
  @Test
  void happyCase() {
    given(gateway.existsByName("baeldung")).willReturn(false);
    given(factory.create("baeldung", "12345")).willReturn(new CommonUser("baeldung", "12345"));
    ...
    then(presenter)
        .should()
        .prepareSuccessView(
            new UserRegisterResponseDto(1L, "baeldung", LocalDateTime.now(clock).toString()));
  }
}
